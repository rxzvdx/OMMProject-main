...
class Question:
    ...
    def getImage(self):
        if self.image:  # Check if image exists
            pathToImage = f"{self.UPLOAD_FOLDER}/{self.image}"
            try:
        ...
        return None  # No image assigned

    def getExplanationImage(self):
        if self.explanationImage:  # Check if explanation image exists
            pathToImage = f"{self.UPLOAD_FOLDER}/{self.explanationImage}"
            try:
                ...
                return None  # Return None if the file is missing
        return None  # No explanation image assigned
